...
class Country extends ControllerBase {
	...
	private function addRecord($params) {
		...
		$datas[] = $this->db->fieldValue(CountryTable::C_DESC,$this->getParam($params,'desc',"")) ;
		$datas[] = $this->db->fieldValue(CountryTable::C_REF,$this->getParam($params,'ref',"")) ;
		$datas[] = $this->db->fieldValue(CountryTable::C_WS_ID,$ws) ;
		$datas[] = $this->db->fieldValue(CountryTable::C_MODIFY_BY,$modifyby) ;
		$datas[] = $this->db->fieldValue(CountryTable::C_CREATE_BY,$modifyby) ;
		$datas[] = $this->db->fieldValue(CountryTable::C_MODIFY_DATE,$modifydate) ;
		$datas[] = $this->db->fieldValue(CountryTable::C_CREATE_DATE,$modifydate) ;
		$datas[] = $this->db->fieldValue(CountryTable::C_ORG_ID,$orgid) ;
		
		try {
			$id = $cls->addRecord($datas) ;
			if ($id > 0) {
				$this->sendJsonResponse(Status::Ok,"Country successfully added to the system.",$id,$this->type);
			} else {
				$this->sendJsonResponse(Status::Error,"Sorry, there is a error in adding new country to the system.",$id, $this->type) ;
			}
		} catch (Exception $e) {
			Log::write('[Country]' . $e->getMessage());
	...
	private function updateRecord($params) {
		if (isset($params['id'])) {
			...
			try {
				...
				$this->sendJsonResponse(Status::Ok,"Country detail successfully updated to the system.",$id,$this->type) ;
			} catch (Exception $e) {
				Log::write('[Country]' . $e->getMessage());
	...
	private function deleteRecord($params) {
		if (isset($params['id'])) {
			...
			try {
				$cls->deleteRecord($id) ; 
				$this->sendJsonResponse(Status::Ok,"Country successfully deleted from the system.","",$this->type);
			} catch (Exception $e) {
				Log::write('[Country]' . $e->getMessage());
	...
	private function getList($params=null) {
		$cls = new CountryClass($this->db) ;
		$filter = $this->db->fieldParam(CountryTable::C_ORG_ID) ;
		$params = array() ;
		$params[] = $this->db->valueParam(CountryTable::C_ORG_ID,$_SESSION[SE_ORGID]) ;
		$rows = $cls->getTable($filter,CountryTable::C_DESC,$params) ;
		$list = "" ;
		foreach ($rows as $row) {
			$id = $row[CountryTable::C_ID] ;
			$list .= "<tr>" ;
			$list .= "<td>" . $id . "</td>" ;
			$list .= "<td>" . $row[CountryTable::C_DESC] . "</td>" ;
			$list .= "<td style='text-align:center'><a href='javascript:' onclick='editCountry(" . $id . ",this)'><img src='image/edit_16.png' title='Edit'></img></a></td>" ;
			$list .= "<td style='text-align:center'><a href='javascript:' onclick='deleteCountry(" . $id . ",this)'><img src='image/delete_16.png' title='Delete'></img></a></td>" ;
			$list .= "</tr>" ;
		}
		unset($rows) ;
		unset($cls) ;
	...
	private function getRecord($params=null) {
		if (isset($params['id'])) {
			...
			if (is_null($row)) {
				...
				$datas = array() ;
				$datas['id'] = $id ;
				$datas['desc'] = $row[CountryTable::C_DESC];
				$datas['ref'] = $row[CountryTable::C_REF] ;
	...
	private function getView() {
		ob_start() ;
		include (PATH_VIEWS . "claims/CountryView.php") ;
	...
	private function getReport($params=null) {
		...
		foreach ($rows as $row) {
			$items = array() ;
			$items[$i][] = $this->createPdfItem($row[CountryTable::C_ID],30) ;
			$items[$i][] = $this->createPdfItem($row[CountryTable::C_DESC],200) ;
			$items[$i][] = $this->createPdfItem($row[CountryTable::C_REF],100) ;
			$items[$nr] = "1" ;
			$datas[] = $items ;
		}
		$cols = array() ;
		...
		$pdf->setReportTitle("Country Listing") ;
