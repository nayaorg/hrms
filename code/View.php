...
function fatalErrorShutdownHandler()
{
	...
	if ($last_error['type'] > 0)
		Log::write('[View]' . $last_error['message'] . " - " . $last_error['file'] . " on line " . $last_error['line']);
}
...
if (!class_exists($page)) {
	$page = "Login" ;
	Log::write("[View]class not found : " . $class . ", page : " . $page) ;
} 
$params = array() ;
$params['type'] = 'v' ;
...
if ($called == FALSE) {
	...
	Log::write("[View]Calling class failed : " . $class . " failed. Page : " . $page,$page) ;
}
//if (class_exists($page)) {
	//$params = array() ;
	//$params['type'] = 'v' ;
